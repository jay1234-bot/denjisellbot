...
ADMIN_IDS = [8746242371, 8333954027, 8666688867]
...
acces_group_id = "-1003901207441"
ACCESS_GROUP_ID_RAW = os.environ.get("ACCESS_GROUP_ID", os.environ.get("acces_group_id", acces_group_id)).strip()
...
SERVER2_MONGODB_URI = os.environ.get(
    "SERVER2_MONGODB_URI",
    "mongodb+srv://danishzain1637:papaspartan@cluster0.xdje3.mongodb.net/server2_bot?retryWrites=true&w=majority&appName=Cluster0",
).strip()
...
_mongo2_client: Optional[MongoClient] = None
_force_link_cache: Dict[int, str] = {}

try:
    ACCESS_GROUP_ID = int(ACCESS_GROUP_ID_RAW) if ACCESS_GROUP_ID_RAW else 0
except ValueError:
    ACCESS_GROUP_ID = 0
...
def get_mongo2_client() -> MongoClient:
    global _mongo2_client
    if not SERVER2_MONGODB_URI:
        raise RuntimeError("SERVER2_MONGODB_URI is not set.")
    if _mongo2_client is None:
        _install_mongo_tls_workaround()
        _install_mongo_socket_ipv4_for_atlas()
        base_opts = _mongo_client_kwargs()
        _mongo2_client = MongoClient(SERVER2_MONGODB_URI, **base_opts)
        _mongo2_client.admin.command("ping")
    return _mongo2_client


def get_mongo2():
    return get_mongo2_client().get_default_database()
...
def next_seq2(counter_name: str) -> int:
    doc = get_mongo2().counters.find_one_and_update(
        {"_id": counter_name},
        {"$inc": {"seq": 1}},
        upsert=True,
        return_document=ReturnDocument.AFTER,
...
def init_db():
    ...
    db.manual_numbers.create_index("target_user_id")
...
def init_db_server2():
    db = get_mongo2()
    db.countries.create_index("code", unique=True)
    db.countries.create_index("id", unique=True)
    db.orders.create_index("status")
    db.orders.create_index("user_id")
...
def resolve_country_from_input(raw: str) -> Optional[Dict[str, str]]:
    val = (raw or "").strip().lower()
    if not val:
        return None
    for code, name, flag in COUNTRIES:
        if val == code.lower() or val == name.lower():
...
def get_dialing_prefix_for_country(code: str) -> str:
    for dial, (cc, _, _) in DIALING_CODE_MAP.items():
        if cc == code:
            return f"+{dial}"
    return ""


def s2_get_country_by_id(country_id: int) -> Optional[Dict[str, Any]]:
    doc = get_mongo2().countries.find_one({"id": country_id})
    return dict(doc) if doc else None


def s2_get_country_by_code(code: str) -> Optional[Dict[str, Any]]:
    doc = get_mongo2().countries.find_one({"code": code})
    return dict(doc) if doc else None


def s2_upsert_country(code: str, name: str, flag: str, dial_code: str, stock: int, price_inr: float) -> Dict[str, Any]:
    db = get_mongo2()
    existing = db.countries.find_one({"code": code})
    if existing:
        db.countries.update_one(
            {"code": code},
            {"$set": {"name": name, "flag": flag, "dial_code": dial_code, "stock_count": int(stock), "price_inr": float(price_inr), "enabled": 1}},
        )
        return dict(db.countries.find_one({"code": code}) or {})
    cid = next_seq2("countries")
    doc = {
        "_id": f"s2_{code}",
        "id": cid,
        "code": code,
...
def s2_list_countries() -> List[Dict[str, Any]]:
    return [dict(x) for x in get_mongo2().countries.find().sort("name", ASCENDING)]


def s2_list_browse_countries() -> List[Dict[str, Any]]:
    return [dict(x) for x in get_mongo2().countries.find({"enabled": 1, "stock_count": {"$gt": 0}}).sort("name", ASCENDING)]


def s2_insert_pending_order(user, country: Dict[str, Any], screenshot_file_id: str) -> int:
    db = get_mongo2()
    oid = next_seq2("orders")
    db.orders.insert_one(
        {
            ...
            "amount_inr": float(country["price_inr"]),
            "server": "server 2",
            "payment_screenshot": screenshot_file_id,
            "status": "pending",
...
def s2_get_order(order_id: int) -> Optional[Dict[str, Any]]:
    doc = get_mongo2().orders.find_one({"_id": order_id})
    return dict(doc) if doc else None


def s2_reject_order_db(order_id: int, reviewer_id: int) -> Optional[Dict[str, Any]]:
    db = get_mongo2()
    ...
    db.orders.update_one(
        {"_id": order_id},
        {"$set": {"status": "rejected", "reviewed_by": reviewer_id, "reviewed_at": now_ist().isoformat()}},
    )
    return dict(order)


def s2_approve_order_payment(order_id: int, reviewer_id: int) -> Optional[Dict[str, Any]]:
    db = get_mongo2()
    now = now_ist().isoformat()
    with get_mongo2_client().start_session() as session:
        with session.start_transaction():
            order = db.orders.find_one({"_id": order_id, "status": "pending"}, session=session)
            if not order:
                return None
            country = db.countries.find_one_and_update(
                {"id": int(order["country_id"]), "stock_count": {"$gt": 0}},
                {"$inc": {"stock_count": -1}},
                return_document=ReturnDocument.AFTER,
                session=session,
            )
            ...
            out = dict(order)
            out["stock_left"] = int(country.get("stock_count") or 0)
            return out
...
def mask_half_phone(phone_number: str) -> str:
    digits = str(phone_number or "").lstrip("+")
    if not digits:
        return "*****"
    visible = max(3, len(digits) // 2)
    return f"+{digits[:visible]}{'*' * max(5, len(digits) - visible)}"


def insert_manual_number(
    phone_number: str,
    country_code: str,
    price_inr: float,
    session_string: str,
    ...
    db = get_mongo()
    mid = next_seq("manual_numbers")
    db.manual_numbers.insert_one(
        {
            "_id": mid,
            "id": mid,
...
def get_manual_number(manual_id: int) -> Optional[Dict[str, Any]]:
    doc = get_mongo().manual_numbers.find_one({"_id": manual_id})
    return dict(doc) if doc else None
...
def _looks_like_python_bot_file(data: bytes) -> bool:
    try:
    ...
    if not stripped:
        return False
    forbidden_starts = ("FROM ", "WORKDIR ", "RUN ", "CMD ", "ENTRYPOINT ", "COPY ")
    if any(stripped.startswith(prefix) for prefix in forbidden_starts):
        return False
    required_markers = ("import asyncio", "from telegram import", "def main(")
...
def download_bot_py_from_github() -> None:
    ...
    with urllib.request.urlopen(req, timeout=120) as resp:
        data = resp.read()
    if not _looks_like_python_bot_file(data):
        sample = data[:120].decode("utf-8", errors="replace").strip()
        raise RuntimeError(f"Downloaded file is not a valid bot.py payload: {sample}")
...
def main_menu_kb():
    return InlineKeyboardMarkup(
        [
            [ibutton("menu.btn_browse", callback_data="browse_servers")],
            [
...
async def _resolve_access_group_join_url(bot: Bot) -> str:
    if not ACCESS_GROUP_ID:
        return ""
    return await _resolve_force_join_url(bot, ACCESS_GROUP_ID)
...
async def send_manual_number_to_access_group(
    context: ContextTypes.DEFAULT_TYPE,
    manual_id: int,
) -> bool:
    if not ACCESS_GROUP_ID:
        return False
    manual = get_manual_number(manual_id)
    if not manual:
        return False
    country = get_country(manual["country_code"]) or {}
    text = (
        "🔓 <b>Number Revealed</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"📱 <b>Number:</b> <code>{mask_half_phone(manual.get('phone_number') or '')}</code>\n"
        f"🌍 <b>Country:</b> {country.get('flag', '')} {escape(country.get('name', 'Unknown'))}\n"
        f"💰 <b>Price:</b> ₹{float(manual.get('price_inr') or 0):.0f}\n"
        f"🖥 <b>Server:</b> {escape(str(manual.get('server') or 'server 1'))}"
    )
    kb = InlineKeyboardMarkup(
        [[ibutton_raw("✅ Get OTP", callback_data=f"manualotp_{manual_id}", icon_slot="check", style="success")]]
    )
    await context.bot.send_message(
        chat_id=ACCESS_GROUP_ID,
        text=text,
        parse_mode="HTML",
        reply_markup=kb,
    )
...
# ─── BROWSE NUMBERS ──────────────────────────────────────────────────────────
async def browse_servers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup(
        [
            [ibutton_raw("🔵 𝐬𝐞𝐫𝐯𝐞𝐫 1", callback_data="browse_0", icon_slot="globe", style="primary")],
            [ibutton_raw("🔴 𝐬𝐞𝐫𝐯𝐞𝐫 2", callback_data="s2_browse_0", icon_slot="globe", style="danger")],
            [ibutton("common.main_menu", callback_data="main_menu")],
        ]
    )
    await safe_edit_callback_message(
        query,
        text="🌐 <b>𝐜ʜᴏᴏ𝐬ᴇ 𝐬ᴇʀᴠᴇʀ</b>\n━━━━━━━━━━━━━━━━━━━━",
        parse_mode="HTML",
        reply_markup=kb,
    )
...
async def s2_browse_numbers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if await guard(update, context):
        return
    page = int(query.data.split("_")[2])
    countries = s2_list_browse_countries()
    if not countries:
        await safe_edit_callback_message(
            query,
            text="❌ <b>𝐬ᴇʀᴠᴇʀ 2 ᴍᴇ ᴀʙʜɪ ᴄᴏᴜɴᴛʀʏ ᴏʀ sᴛᴏᴄᴋ ᴀᴠᴀɪʟᴀʙʟᴇ ɴᴀʜɪ ʜᴀɪ.</b>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[ibutton_raw("🔙 𝐛ᴀᴄᴋ", callback_data="browse_servers", style="primary")]]),
        )
        return
    per_page = 6
    total = len(countries)
    ...
    lines = ["🔴 <b>𝐬ᴇʀᴠᴇʀ 2 ᴄᴏᴜɴᴛʀɪᴇs</b>\n━━━━━━━━━━━━━━━━━━━━"]
    buttons = []
    for c in chunk:
        lines.append(f"{c['flag']} <b>{escape(c['name'])}</b> • 📞 {escape(c.get('dial_code') or 'N/A')} • 📦 {int(c.get('stock_count') or 0)} • ₹{float(c.get('price_inr') or 0):.0f}")
        buttons.append([ibutton_raw(f"{c['flag']} {c['name']} • ₹{float(c.get('price_inr') or 0):.0f}", callback_data=f"s2_country_{c['id']}", style="success")])
    nav = []
    if page > 0:
        nav.append(ibutton_raw("◀️ 𝐩ʀᴇᴠ", callback_data=f"s2_browse_{page-1}", style="primary"))
    if page < pages - 1:
        nav.append(ibutton_raw("𝐧ᴇxᴛ ▶️", callback_data=f"s2_browse_{page+1}", style="primary"))
    if nav:
        buttons.append(nav)
    buttons.append([ibutton_raw("🔙 𝐬ᴇʀᴠᴇʀs", callback_data="browse_servers", style="danger")])
    await safe_edit_callback_message(
        query,
        text="\n".join(lines),
        parse_mode="HTML",
...
async def s2_country_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if await guard(update, context):
        return
    cid = int(query.data.split("_")[2])
    c = s2_get_country_by_id(cid)
    if not c:
        await query.answer("Country not found.", show_alert=True)
        return
    text = (
        f"🔴 <b>𝐬ᴇʀᴠᴇʀ 2 ᴅᴇᴛᴀɪʟs</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"{c['flag']} <b>{escape(c['name'])}</b>\n"
        f"📞 <b>ᴅɪᴀʟ ᴄᴏᴅᴇ:</b> {escape(c.get('dial_code') or 'N/A')}\n"
        f"📦 <b>sᴛᴏᴄᴋ:</b> {int(c.get('stock_count') or 0)}\n"
        f"💰 <b>ᴘʀɪᴄᴇ:</b> ₹{float(c.get('price_inr') or 0):.0f}"
    )
    kb = InlineKeyboardMarkup(
        [
            [ibutton_raw("🔴 𝐩ᴀʏ ɴᴏᴡ", callback_data=f"s2_pay_{cid}", style="danger")],
            [ibutton_raw("🔙 𝐛ᴀᴄᴋ", callback_data="s2_browse_0", style="primary")],
        ]
    )
    await safe_edit_callback_message(query, text=text, parse_mode="HTML", reply_markup=kb)


async def s2_pay_method(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if await guard(update, context):
        return
    cid = int(query.data.split("_")[2])
    c = s2_get_country_by_id(cid)
    if not c or int(c.get("stock_count") or 0) <= 0:
        await query.answer("Out of stock.", show_alert=True)
        return
    context.user_data["s2_buy_country_id"] = cid
    note = f"Server2 {c['name']}"
    qr_buf = generate_upi_qr(float(c["price_inr"]), note)
    caption = t("buy.pay_caption", amount=float(c["price_inr"]), upi=escape(UPI_ID))
    kb = InlineKeyboardMarkup(
        [
            [ibutton_raw("📤 𝐮ᴘʟᴏᴀᴅ sᴄʀᴇᴇɴsʜᴏᴛ", callback_data=f"s2_upload_{cid}", style="success")],
            [ibutton_raw("🔙 𝐛ᴀᴄᴋ", callback_data=f"s2_country_{cid}", style="primary")],
        ]
    )
...
async def s2_upload_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if await guard(update, context):
        return
    cid = int(query.data.split("_")[2])
    context.user_data["s2_buy_country_id"] = cid
    context.user_data["awaiting_s2_buy_screenshot"] = True
    await query.edit_message_caption(
        caption="📤 <b>sᴄʀᴇᴇɴsʜᴏᴛ sᴇɴᴅ ᴋᴀʀᴏ ᴘᴀʏᴍᴇɴᴛ ᴠᴇʀɪꜰʏ ᴋᴇ ʟɪʏᴇ.</b>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([[ibutton_raw("🔙 𝐜ᴀɴᴄᴇʟ", callback_data=f"s2_country_{cid}", style="danger")]]),
    )
...
async def screenshot_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Server 2 buy screenshot
    if context.user_data.get("awaiting_s2_buy_screenshot"):
        context.user_data.pop("awaiting_s2_buy_screenshot", None)
        cid = context.user_data.get("s2_buy_country_id")
        c = s2_get_country_by_id(int(cid)) if cid else None
        if not c:
        ...
        if not file_id:
            ...
            return
        order_id = s2_insert_pending_order(user, c, file_id)
        clickable_name = f"<a href='tg://user?id={user.id}'>{escape((user.first_name or 'User'))}</a>"
        uname = f"@{escape(user.username)}" if user.username else "<i>not_set</i>"
        text = (
            f"🔴 <b>𝐬ᴇʀᴠᴇʀ 2 ᴘᴀʏᴍᴇɴᴛ</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🌍 <b>ᴄᴏᴜɴᴛʀʏ:</b> {c['flag']} {escape(c['name'])}\n"
            f"👤 <b>ᴜsᴇʀɴᴀᴍᴇ:</b> {uname}\n"
            f"🧾 <b>ɴᴀᴍᴇ:</b> {clickable_name}\n"
            f"🆔 <b>ᴜsᴇʀ ɪᴅ:</b> <code>{user.id}</code>\n"
            f"💰 <b>ᴀᴍᴏᴜɴᴛ:</b> ₹{float(c['price_inr']):.0f}\n"
            f"🧺 <b>ᴏʀᴅᴇʀ:</b> <code>#{order_id}</code>"
        )
        kb = InlineKeyboardMarkup([[
            ibutton_raw("✅ 𝐚ᴘᴘʀᴏᴠᴇ", callback_data=f"s2_approve_order_{order_id}", style="success"),
            ibutton_raw("❌ 𝐫ᴇᴊᴇᴄᴛ", callback_data=f"s2_reject_order_{order_id}", style="danger"),
        ]])
        try:
            await context.bot.send_photo(chat_id=ADMIN_GROUP_ID, photo=file_id, caption=text, parse_mode="HTML", reply_markup=kb)
        except Exception as e:
            logger.error(f"Failed to forward server2 payment: {e}")
        await update.message.reply_text(
            "✅ <b>ᴘᴀʏᴍᴇɴᴛ sᴜʙᴍɪᴛᴛᴇᴅ.</b>\n<b>ᴀᴅᴍɪɴ ᴠᴇʀɪꜰʏ ᴋʀᴇɢᴀ.</b>",
            parse_mode="HTML",
            reply_markup=main_menu_kb(),
        )
        return
...
async def manual_get_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer("Fetching OTP...")
    manual_id = int(query.data.split("_")[1])
    manual = get_manual_number(manual_id)
    if not manual:
        await query.answer("Number data not found.", show_alert=True)
        return
    allowed_user_id = int(manual.get("target_user_id") or 0)
    if query.from_user.id not in ADMIN_IDS and query.from_user.id != allowed_user_id:
        await query.answer("Access restricted.", show_alert=True)
        return

    await safe_edit_callback_message(
        query,
        text="⏳ <b>Fetching the latest OTP...</b>",
        parse_mode="HTML",
        reply_markup=None,
    )
    otp_code = None
    error_msg = None
    client = TelegramClient(StringSession(manual["session_string"]), API_ID, API_HASH)
    try:
        await client.connect()
        otp_code = await _fetch_otp(client)
    except FloodWaitError as e:
        error_msg = f"⏳ Please wait {e.seconds} seconds before trying again."
    except Exception as e:
        logger.error(f"Manual OTP fetch error: {e}")
        if "session" in str(e).lower() or "auth" in str(e).lower():
            error_msg = "❌ This session has expired."
        else:
            error_msg = "⚠️ Could not fetch OTP right now. Please try again."
    finally:
        try:
            await client.disconnect()
        except Exception:
    ...
    if error_msg:
        ...
        text = (
            "✅ <b>Latest OTP</b>\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            f"🔑 <b>OTP:</b> <code>{escape(str(otp_code or 'Not found'))}</code>\n"
            f"📱 <b>Number:</b> <code>{mask_half_phone(manual.get('phone_number') or '')}</code>\n"
            f"🌍 <b>Country:</b> {country.get('flag', '')} {escape(country.get('name', 'Unknown'))}\n"
            f"⏱ <b>Fetched At:</b> {now_ist().strftime('%H:%M:%S IST')}"
        )
    kb = InlineKeyboardMarkup(
        [[ibutton_raw("✅ Get OTP", callback_data=f"manualotp_{manual_id}", icon_slot="check", style="success")]]
    )
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Admin: /sendnumber flow - number
    if context.user_data.get("awaiting_sendnumber_phone"):
        if not is_admin(user.id):
            return
        phone = update.message.text.strip()
        if not phone.startswith("+"):
            await update.message.reply_text("❌ Please send the number in international format. Example: +919876543210")
            return
        context.user_data.pop("awaiting_sendnumber_phone", None)
        context.user_data["sendnumber_phone"] = phone
        context.user_data["awaiting_sendnumber_country"] = True
        await update.message.reply_text(
            "🌍 Now send the country name, ISO code, or dialing code.\nExamples: India / IN / +91"
        )
        return

    # Admin: /sendnumber flow - country
    if context.user_data.get("awaiting_sendnumber_country"):
        if not is_admin(user.id):
            return
        raw = update.message.text.strip()
        match = None
        if raw.startswith("+") or raw.isdigit():
            dial_match = lookup_dialing_code(raw)
            if dial_match:
                match = {"code": dial_match[0], "name": dial_match[1], "flag": dial_match[2]}
        if not match:
            match = resolve_country_from_input(raw)
        if not match:
            await update.message.reply_text("❌ Country not found. Please send a valid country name, ISO code, or dialing code.")
            return
        context.user_data.pop("awaiting_sendnumber_country", None)
        context.user_data["sendnumber_country"] = match["code"]
        s2_country = s2_get_country_by_code(match["code"])
        price_inr = float((s2_country or {}).get("price_inr") or 0)
        dial = get_dialing_prefix_for_country(match["code"])
        if price_inr > 0:
            context.user_data["sendnumber_price"] = price_inr
            context.user_data["awaiting_sendnumber_session"] = True
            await update.message.reply_text(
                f"✅ Country detected: {match['flag']} {match['name']} ({dial or 'N/A'})\n"
                f"💰 Price fetched: ₹{price_inr:.0f}\n\n"
                "🔐 Please send the Telethon session string:"
            )
        else:
            context.user_data["awaiting_sendnumber_manual_price"] = True
            await update.message.reply_text(
                f"✅ Country detected: {match['flag']} {match['name']} ({dial or 'N/A'})\n"
                "⚠️ No price found in Server 2 Mongo.\n"
                "💰 Please add price manually in INR:"
            )
        return

    # Admin: /sendnumber flow - manual price fallback
    if context.user_data.get("awaiting_sendnumber_manual_price"):
        if not is_admin(user.id):
            return
        try:
            price_inr = float(update.message.text.strip())
        except ValueError:
            await update.message.reply_text("❌ Invalid price. Please send a numeric value.")
            return
        if price_inr <= 0:
            await update.message.reply_text("❌ Price must be greater than 0.")
            return
        context.user_data.pop("awaiting_sendnumber_manual_price", None)
        context.user_data["sendnumber_price"] = price_inr
        context.user_data["awaiting_sendnumber_session"] = True
        await update.message.reply_text(
            f"✅ Manual price saved: ₹{price_inr:.0f}\n\n"
            "🔐 Please send the Telethon session string:"
        )
        return

    # Admin: /sendnumber flow - session
    if context.user_data.get("awaiting_sendnumber_session"):
        if not is_admin(user.id):
            return
        session_string = update.message.text.strip()
        if len(session_string) < 10:
            await update.message.reply_text("❌ Invalid session string. Please send the full Telethon session string.")
            return
        context.user_data.pop("awaiting_sendnumber_session", None)
        context.user_data["sendnumber_session"] = session_string
        context.user_data["awaiting_sendnumber_userid"] = True
        await update.message.reply_text("👤 Now send the target user ID who should be allowed to fetch OTP:")
        return

    # Admin: /sendnumber flow - target user id
    if context.user_data.get("awaiting_sendnumber_userid"):
        if not is_admin(user.id):
            return
        try:
            ...
            return
        phone = context.user_data.pop("sendnumber_phone", "")
        country_code = context.user_data.pop("sendnumber_country", "")
        session_string = context.user_data.pop("sendnumber_session", "")
        price_inr = float(context.user_data.pop("sendnumber_price", 0))
        context.user_data.pop("awaiting_sendnumber_userid", None)
        if not phone or not country_code or not session_string:
            await update.message.reply_text("❌ Session expired. Please run /sendnumber again.")
            return
        manual_id = insert_manual_number(
            phone_number=phone,
            country_code=country_code,
            price_inr=price_inr,
            session_string=session_string,
        ...
        sent = False
        try:
            sent = await send_manual_number_to_access_group(context, manual_id)
        except Exception as e:
            logger.error(f"Failed to send manual number to access group: {e}")
        country = get_country(country_code) or {}
        await update.message.reply_text(
            f"✅ Number sent successfully.\n"
            f"📱 Number: {mask_half_phone(phone)}\n"
            f"🌍 Country: {country.get('flag', '')} {country.get('name', 'Unknown')}\n"
            f"💰 Price: ₹{price_inr:.0f}\n"
            f"👤 Allowed User ID: {target_user_id}\n"
            f"📤 Access Group Delivery: {'Done' if sent else 'Failed'}"
        )
        return

    # Admin2: country name/code input
    if context.user_data.get("awaiting_s2_country_name"):
        if not is_admin(user.id):
            return
        context.user_data.pop("awaiting_s2_country_name", None)
        match = resolve_country_from_input(update.message.text.strip())
        if not match:
            context.user_data["awaiting_s2_country_name"] = True
            await update.message.reply_text("❌ Country not found. Please send a valid country name or ISO code.")
            return
        context.user_data["s2_pending_country"] = match
        context.user_data["awaiting_s2_stock"] = True
        dial = get_dialing_prefix_for_country(match["code"])
        await update.message.reply_text(
            f"✅ Detected: {match['flag']} {match['name']} ({dial or 'N/A'})\nNow send the stock count:",
        )
        return

    # Admin2: stock input
    if context.user_data.get("awaiting_s2_stock"):
        if not is_admin(user.id):
            return
        try:
            stock = int(update.message.text.strip())
        except ValueError:
            await update.message.reply_text("❌ Invalid stock value. Please send a number.")
            return
        if stock < 0:
            await update.message.reply_text("❌ stock cannot be negative.")
            return
        context.user_data.pop("awaiting_s2_stock", None)
        context.user_data["s2_pending_stock"] = stock
        context.user_data["awaiting_s2_price"] = True
        await update.message.reply_text("💰 Now send the country price in INR:")
        return

    # Admin2: price input
    if context.user_data.get("awaiting_s2_price"):
        if not is_admin(user.id):
            return
        try:
            ...
            return
        pending_country = context.user_data.pop("s2_pending_country", None) or {}
        stock = int(context.user_data.pop("s2_pending_stock", 0))
        context.user_data.pop("awaiting_s2_price", None)
        if not pending_country:
            await update.message.reply_text("❌ Session expired. Please use /admin2 again.")
            return
        dial = get_dialing_prefix_for_country(pending_country["code"])
        row = s2_upsert_country(
            pending_country["code"],
            pending_country["name"],
            pending_country["flag"],
            dial,
            stock,
            price,
        )
        await update.message.reply_text(
            f"✅ Server 2 country saved successfully\n"
            f"{row['flag']} {row['name']} ({row.get('dial_code') or 'N/A'})\n"
            f"Stock: {int(row.get('stock_count') or 0)}\n"
            f"Price: ₹{float(row.get('price_inr') or 0):.0f}",
            reply_markup=admin2_main_kb(),
        )
        return
...
async def s2_approve_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if query.from_user.id not in ADMIN_IDS:
        await query.answer("Access rejected. You are not an admin.", show_alert=True)
        return
    await query.answer()
    order_id = int(query.data.split("_")[3])
    order = s2_approve_order_payment(order_id, query.from_user.id)
    if not order:
        await query.edit_message_caption(caption=(query.message.caption or "") + "\n⚠️ already processed / no stock.")
        return

    access_join_url = await _resolve_access_group_join_url(context.bot)
    join_rows = []
    if access_join_url:
        join_rows.append([InlineKeyboardButton("Join Access Group", url=access_join_url)])
    join_kb = InlineKeyboardMarkup(join_rows) if join_rows else None
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                "━━━━━━━━━━━━━━━━━━━━\n"
                "Your Server 2 payment has been verified.\n\n"
                "Because Telegram is taking action against bots, we do not deliver Server 2 access here.\n\n"
                "<b>Why join the access group?</b>\n"
                "• No scam\n"
                "• Good accounts\n"
                "• Cheap accounts\n"
                "• Trusted delivery\n"
                "• Fast access after approval\n\n"
                "Please join the access group below to receive your ID access."
            ),
            parse_mode="HTML",
    ...
    if ACCESS_GROUP_ID:
        mention_name = f"<a href='tg://user?id={order['user_id']}'>{escape(order.get('user_first_name') or 'User')}</a>"
        uname = f"@{escape(order.get('username') or '')}" if order.get("username") else "<i>not_set</i>"
        pending_text = (
            "🟡 <b>𝐩ᴇɴᴅɪɴɢ ᴀᴄᴄᴇs ᴀᴘᴘʀᴏᴠᴀʟ</b>\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            f"👤 <b>ᴜsᴇʀɴᴀᴍᴇ:</b> {uname}\n"
            f"🧾 <b>ɴᴀᴍᴇ:</b> {mention_name}\n"
            f"🆔 <b>ᴜsᴇʀ ɪᴅ:</b> <code>{order['user_id']}</code>\n"
            f"🌍 <b>ᴄᴏᴜɴᴛʀʏ:</b> {order.get('country_flag', '')} {escape(order.get('country_name', 'N/A'))}\n"
            f"🖥 <b>sᴇʀᴠᴇʀ:</b> server 2\n"
            f"💰 <b>ɪᴅ ᴘʀɪᴄᴇ:</b> ₹{float(order.get('amount_inr') or 0):.0f}\n"
            f"📦 <b>sᴛᴏᴄᴋ ʟᴇꜰᴛ:</b> {int(order.get('stock_left') or 0)}"
        )
        kb = InlineKeyboardMarkup([[ibutton_raw("✅ 𝐚ᴘᴘʀᴏᴠᴇ ᴀᴄᴄᴇs", callback_data=f"s2_access_approve_{order_id}", style="success")]])
        try:
            await context.bot.send_message(chat_id=ACCESS_GROUP_ID, text=pending_text, parse_mode="HTML", reply_markup=kb)
        except Exception as e:
            logger.error(f"Failed to send pending access msg: {e}")
...
async def s2_reject_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if query.from_user.id not in ADMIN_IDS:
        await query.answer("Access rejected. You are not an admin.", show_alert=True)
        return
    await query.answer()
    order_id = int(query.data.split("_")[3])
    order = s2_reject_order_db(order_id, query.from_user.id)
    if not order:
        await query.edit_message_caption(caption=(query.message.caption or "") + "\n⚠️ already processed.")
        return
    try:
        await context.bot.send_message(chat_id=order["user_id"], text="❌ Payment rejected. Please contact admin or support.")
    except Exception:
        pass
    await query.edit_message_caption(caption=(query.message.caption or "") + f"\n❌ rejected by @{query.from_user.username or query.from_user.id}")


async def s2_access_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if query.from_user.id not in ADMIN_IDS:
        await query.answer("Access rejected. You are not an admin.", show_alert=True)
        return
    ...
    order_id = int(query.data.split("_")[3])
    db2 = get_mongo2()
    order = db2.orders.find_one({"_id": order_id, "status": "approved", "access_status": "pending"})
    if not order:
        await query.answer("already processed", show_alert=True)
        return
    db2.orders.update_one(
        {"_id": order_id},
        {"$set": {"access_status": "given", "access_reviewed_by": query.from_user.id, "access_reviewed_at": now_ist().isoformat()}},
    )
    final_text = (
        "✅ <b>𝐚ᴄᴄᴇs ɢɪᴠᴇɴ</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"👤 <b>ᴜsᴇʀɴᴀᴍᴇ:</b> @{escape(order.get('username') or 'not_set')}\n"
        f"🆔 <b>ᴜsᴇʀ ɪᴅ:</b> <code>{order['user_id']}</code>\n"
        f"🌍 <b>ᴄᴏᴜɴᴛʀʏ:</b> {order.get('country_flag', '')} {escape(order.get('country_name', 'N/A'))}\n"
        f"🖥 <b>sᴇʀᴠᴇʀ:</b> server 2\n"
        f"💰 <b>ᴀᴍᴏᴜɴᴛ:</b> ₹{float(order.get('amount_inr') or 0):.0f}"
    )
    await query.edit_message_text(final_text, parse_mode="HTML", reply_markup=None)

    success_text = (
        "🎉 <b>sᴜᴄᴄᴇssꜰᴜʟʟʏ ᴘᴜʀᴄʜᴀsᴇᴅ ɪᴅ</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"📱 <b>ɴᴜᴍʙᴇʀ:</b> <code>**********</code>\n"
        f"🌍 <b>ᴄᴏᴜɴᴛʀʏ:</b> {order.get('country_flag', '')} {escape(order.get('country_name', 'N/A'))}\n"
        f"💰 <b>ᴀᴍᴏᴜɴᴛ:</b> ₹{float(order.get('amount_inr') or 0):.0f}\n"
        f"🖥 <b>sᴇʀᴠᴇʀ:</b> server 2"
    )
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🛒 ʙᴜʏ ɴᴏᴡ", url="https://t.me/xr_otp_bot?start=buy")]])
    try:
        await context.bot.send_message(chat_id=LOG_GROUP_ID, text=success_text, parse_mode="HTML", reply_markup=kb)
    except Exception as e:
        logger.error(f"Failed to send server2 success log: {e}")
...
# ─── ADMIN2 PANEL (SERVER 2) ─────────────────────────────────────────────────
def admin2_main_kb():
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("➕ add country", callback_data="s2_admin_add_country")],
            [InlineKeyboardButton("📋 view countries", callback_data="s2_admin_list_0")],
            [InlineKeyboardButton("🔙 close", callback_data="admin_close")],
        ]
    )


async def admin2_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("❌ Not authorized.")
        return
    await update.message.reply_text("🔴 *Server 2 Admin Panel*", parse_mode="Markdown", reply_markup=admin2_main_kb())


async def sendnumber_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("❌ Not authorized.")
        return
    context.user_data.pop("awaiting_sendnumber_country", None)
    context.user_data.pop("awaiting_sendnumber_session", None)
    context.user_data.pop("awaiting_sendnumber_userid", None)
    context.user_data["awaiting_sendnumber_phone"] = True
    await update.message.reply_text(
        "📱 Send the number in international format.\nExample: +919876543210"
    )


async def s2_admin_add_country(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    context.user_data["awaiting_s2_country_name"] = True
    await query.edit_message_text(
        "🌍 Send country name or ISO code\nExample: India / IN",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 back", callback_data="s2_admin_list_0")]]),
    )


async def s2_admin_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not is_admin(query.from_user.id):
        return
    page = int(query.data.split("_")[3])
    rows = s2_list_countries()
    if not rows:
        await query.edit_message_text("❌ No countries added in Server 2 yet.", reply_markup=admin2_main_kb())
        return
    per_page = 8
    pages = max(1, (len(rows) + per_page - 1) // per_page)
    page = max(0, min(page, pages - 1))
    chunk = rows[page * per_page:(page + 1) * per_page]
    lines = ["🔴 *Server 2 Countries*"]
    for c in chunk:
        lines.append(
            f"{c['flag']} {c['name']} ({c['dial_code'] or 'N/A'}) | Stock: {int(c.get('stock_count') or 0)} | Price: ₹{float(c.get('price_inr') or 0):.0f}"
        )
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("◀️ prev", callback_data=f"s2_admin_list_{page-1}"))
    if page < pages - 1:
        nav.append(InlineKeyboardButton("next ▶️", callback_data=f"s2_admin_list_{page+1}"))
    buttons = []
    if nav:
        buttons.append(nav)
    buttons.append([InlineKeyboardButton("➕ add country", callback_data="s2_admin_add_country")])
    buttons.append([InlineKeyboardButton("🔙 menu", callback_data="s2_admin_menu")])
    await query.edit_message_text("\n".join(lines), parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(buttons))


async def s2_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not is_admin(query.from_user.id):
        return
    await query.edit_message_text("🔴 *Server 2 Admin Panel*", parse_mode="Markdown", reply_markup=admin2_main_kb())
...
def main():
    ...
    app.add_handler(CommandHandler("admin2", admin2_cmd))
    app.add_handler(CommandHandler("sendnumber", sendnumber_cmd))
    app.add_handler(CommandHandler("update", update_cmd))
    app.add_handler(CommandHandler("skip", skip_cmd))
    # Main navigation
    app.add_handler(CallbackQueryHandler(main_menu_cb, pattern="^main_menu$"))
    app.add_handler(CallbackQueryHandler(browse_servers, pattern="^browse_servers$"))
    app.add_handler(CallbackQueryHandler(browse_numbers, pattern=r"^browse_\d+$"))
    app.add_handler(CallbackQueryHandler(s2_browse_numbers, pattern=r"^s2_browse_\d+$"))
    app.add_handler(CallbackQueryHandler(s2_country_detail, pattern=r"^s2_country_\d+$"))
    app.add_handler(CallbackQueryHandler(s2_pay_method, pattern=r"^s2_pay_\d+$"))
    app.add_handler(CallbackQueryHandler(s2_upload_prompt, pattern=r"^s2_upload_\d+$"))
    ...
    app.add_handler(CallbackQueryHandler(manual_get_otp, pattern=r"^manualotp_\d+$"))
    ...
    app.add_handler(CallbackQueryHandler(s2_approve_order, pattern=r"^s2_approve_order_\d+$"))
    app.add_handler(CallbackQueryHandler(s2_reject_order, pattern=r"^s2_reject_order_\d+$"))
    app.add_handler(CallbackQueryHandler(s2_access_approve, pattern=r"^s2_access_approve_\d+$"))
    ...
    app.add_handler(CallbackQueryHandler(s2_admin_menu, pattern="^s2_admin_menu$"))
    app.add_handler(CallbackQueryHandler(s2_admin_add_country, pattern="^s2_admin_add_country$"))
    app.add_handler(CallbackQueryHandler(s2_admin_list, pattern=r"^s2_admin_list_\d+$"))
